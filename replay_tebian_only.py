# -*- coding: utf-8 -*-
"""
Quick replay script for 特变电工 only
"""
import sys, os
...
# Test indices 10-20 (09:40-09:50)
for i in range(10, 21):
    ...
    sell_factors = diag.get("sell_factors", {})
    buy_factors = diag.get("buy_factors", {})
    sell_block = diag.get("sell_block_reasons", [])
    buy_block = diag.get("buy_block_reasons", [])
    if sell_factors or buy_factors:
        print(f"  sell_factors: {sell_factors}")
        print(f"  buy_factors: {buy_factors}")
    ...
    print(f"  market_state: {diag.get('market_state', 'N/A')}")
    print(f"  sell_threshold: {diag.get('sell_threshold', 'N/A')}")
    print(f"  buy_threshold: {diag.get('buy_threshold', 'N/A')}")
