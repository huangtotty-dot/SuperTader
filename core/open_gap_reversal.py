...
# ── 市场代理池（冻结，2026-09-24）——「大盘」用什么算的**单一真源** ──────────────
# 规则里 `mkt_gap = 池内 gap 的中位数`，而「池」由调用方给。若调用方只把自己要交易的池
# 传进来 ⇒ **自指**：2026-09-24 实测与预注册的 981 面板中位在 31 天里 9 天符号相反、
# 腿集交集仅 36/141（等于跑了另一条规则）。故本模块**冻结一份独立的代理名单**，live 与
# 回测都从这里取，杜绝各引一份。
#
# 名单来历：Stage14 证「随机 10 只即可」（k=10 的净均 p5 仍 ≥ +0.20%/腿）；
# Stage18 把**固定、确定性、非绩效筛选**的候选逐一验过，取「面板内 6 位代码字典序最小 20 只
# 且排除篮子」——与全样本中位日度相关 **0.972**、符号一致 **85.8%**（候选里最佳），
# 且**完全不按业绩选**（可辩护性最强）。
# ⚠️ 改动此名单 = 换代理 = 须重跑 Stage18 的保真度/稳健性检验。
MARKET_PROXY_CODES = (
    "000001", "000021", "000032", "000034", "000060",
    "000062", "000063", "000066", "000070", "000155",
    "000158", "000166", "000301", "000338", "000408",
    "000426", "000506", "000510", "000530", "000532",
)
