# -*- coding: utf-8 -*-
"""
merge_baseline_live.py — v1.1.0 同簿记对照组(baseline+活簿记)合并 + summary 重算
由 merge_degraded.py 适配: ROOT=v110_baseline_live, MODES=["baseline"]。
严格校验(逐行解析/日期⊆周段/去重), summary(p1/closed_loop) 用 harness 函数在统一口径 day_bars 上重算。
"""
...
BASE = Path(r"E:\06_T")
MODE_TAG = "baseline_live"
ROOT = BASE / "t_io/validation/v110_baseline_live"
PARTS = ROOT / "parts"
...
def main():
    ...
    all_dates = sorted({s["ts"][:10] for s in collect("baseline")[0]})
    day_bars_cache = {}
    for d in all_dates:
        day_bars = {}
        for c in CODES:
    ...
    for mode in MODES:
        ...
        summary = {"total": len(signals), "wins": wins, "fails": fails, "voids": voids,
                   ...
                   "form": "v1.1.0_baseline_live(趋势层全关, 同v1.1.0代码+config, 活簿记, t55阈值)",
