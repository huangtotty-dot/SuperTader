# -*- coding: utf-8 -*-
"""
merge_variant_a.py — v1.1.1 变体A实验两组合并 + summary 重算(仿 merge_degraded.py)
两组: control(仅键修复) / variantA(键修复+below&slope>=0放行), parts 内文件统一为 *_v102.*
...
def main():
    ...
    for group in GROUPS:
        ...
        summary = {"total": len(signals), "wins": wins, "fails": fails, "voids": voids,
                   ...
                   "form": f"v1.1.1_{group}(键修复+真实日上下文" + (", 变体A: below_ma5_weak&slope>=0放行" if group == "variantA" else ", 门控现状语义") + ")",
