# -*- coding: utf-8 -*-
"""
run_ab_threshold.py — X9 记录阈值阶梯实测驱动
...
ROOT = BASE / f"t_io/validation/v109_threshold/t{SELL_TH.replace('.', '_')}"
PARTS = ROOT / "parts"
...
CHILD_ENV["T_NOTIFY_SELL"] = SELL_TH
CHILD_ENV["T_NOTIFY_SELL_EARLY"] = EARLY_TH
...
def main():
    ...
    log(f"START TH={SELL_TH}/{EARLY_TH} remaining={len(todo)} at {time.strftime('%H:%M:%S')}")
    if not todo:
        log("NOTHING_TODO")
        return 0
    t_start = time.time()
